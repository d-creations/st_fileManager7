...
export const  IBaseDivView  = createDecorator<IBaseDivView>('IBaseDivView');


export interface IBaseDivView{
    closeFileView()
    openFileView()
    moveBar(x: number) // Added missing method from implementation
}

type SuccessHandler = (event: MouseEvent) => void;
type endHandler = () => void;

export class BaseDivView implements IBaseDivView{
